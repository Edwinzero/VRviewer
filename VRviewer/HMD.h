...
private:
	...
	// hhmd render
	void RenderSceneToEye(vr::Hmd_Eye eye, void (*renderFuc)(float*)) {
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		glEnable(GL_DEPTH_TEST);
		// draw scene for the eye
		{
			glm::mat4 vive_to_eye = GetCurrentViewProjectionMatrix(eye);
			renderFuc(glm::value_ptr(vive_to_eye));
		}

		bool isInputCapturedByAnotherProcess = m_HMD->IsInputFocusCapturedByAnotherProcess();
		...
		// ----- Render Model rendering ----- (this program should encapsulate int HMD)
		{
			...
			for (uint32_t unTrackedDevice = 0; unTrackedDevice < vr::k_unMaxTrackedDeviceCount; unTrackedDevice++)
			{
				...
				if (isInputCapturedByAnotherProcess && m_HMD->GetTrackedDeviceClass(unTrackedDevice) == vr::TrackedDeviceClass_Controller)
					continue;
	...
	// hhmd render
	void RenderVRStereoTargets(void(*renderFuc)(float*)) {
		glClearColor(0.15f, 0.15f, 0.18f, 1.0f);
		glEnable(GL_MULTISAMPLE);
		glBindFramebuffer(GL_FRAMEBUFFER, m_leftEye.m_framebuffer);
		glViewport(0, 0, m_RenderWidth, m_RenderHeight);
		//glClearColor(1.f, 0.f, 0.f, 1.0f);
		RenderSceneToEye(vr::Eye_Left, renderFuc);
		...
		RenderSceneToEye(vr::Eye_Right, renderFuc);
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
		glDisable(GL_MULTISAMPLE);
