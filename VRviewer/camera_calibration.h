...
void CalibrateCameraWithChessboard(vector< vector< cv::Point3f > > &objPoints,
	const vector< vector< cv::Point2f > > &imgPoints, cv::Size imageSize, const Pattern &pattern,
	CameraIntrinsic &intr, vector< RigidTransform > &extr, int flag = CV_CALIB_FIX_K4 | CV_CALIB_FIX_K5
) {
	if (imgPoints.empty()) {
...
void UndistortImage(const cv::Mat &src, cv::Mat &dst, const CameraIntrinsic &intr, int interpolation = cv::INTER_NEAREST) {
...
public:
	...
	void Calibrate(CameraParameter &camera, const Pattern &pattern2d, vector< vector< cv::Point3f > > &objPoints, vector< RigidTransform > &m) {
		// intr
		CameraIntrinsic temp;
		Moca::CalibrateCameraWithChessboard(objPoints, imgPoints, imgSize, pattern2d, temp, m);
		//Moca::CalibrateCameraWithChessboard(objPoints, imgPoints, imgSize, pattern2d, camera.intr, m);
	}
};
